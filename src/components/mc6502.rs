...
impl CPU6502 {
    ...
    fn izx(&mut self) -> () {
        self.PC += 1;
        let a = (self.read(self.PC) + self.X) & 0xff;
    ...
    fn abx(&mut self) -> () {
        self.PC += 1;
        let mut paddr = self.read(self.PC);
        self.PC += 1;
    ...
    // Carry
    fn fnzc(&mut self, v: u8) -> () {
        self.Z = if (v & 0xff) == 0 { 1 } else { 0 };
        self.N = if (v & 0x80) != 0 { 1 } else { 0 };
        self.C = if (v & 0x100) != 0 { 1 } else { 0 };
    }

    fn branch(&mut self, v: u8) -> () {
    ...
    fn adc(&mut self) -> () {
        ...
        if self.D != 0 {
            ...
            self.V = if (!(self.A ^ v) & (self.A ^ (ah << 4)) & 0x80) != 0 {
                1
            } else {
                0
            };
            ...
            self.V = if (!(self.A ^ v) & (self.A ^ r) & 0x80) != 0 {
                1
            } else {
                0
            };
    ...
    fn cmp(&mut self) -> () {
        let addr = self.read(self.addr);
        self.fnzb(self.A - addr);
    }

    fn cpx(&mut self) -> () {
        let addr = self.read(self.addr);
        self.fnzb(self.X - addr);
    }

    fn cpy(&mut self) -> () {
        let addr = self.read(self.addr);
        self.fnzb(self.Y - addr);
    }
    ...
    fn sbc(&mut self) -> () {
        ...
        if self.D != 0 {
            ...
            let mut ah = (self.A >> 4) - (v >> 4) - (if al < 0 { 1 } else { 0 });
            self.Z = if (r & 0xff) == 0 { 1 } else { 0 };
            self.N = if (r & 0x80) != 0 { 1 } else { 0 };
